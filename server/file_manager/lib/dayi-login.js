...
function create_new_user_set_cookie(req,res,new_user_name){
  ...
  logger.info('[dayi-info]new_user:'+new_user_name)
...
function renew_cookie(req,res,user_name){
  logger.info('[dayi-info]renew_cookie:'+user_name)
  ...
  if (Object.keys(req.cookies).length !== 0) {
    if (req.cookies.hasOwnProperty(cookie_key)) {
      ...
      if (result) {
        ...
        logger.info('[dayi-info]重复登录操作: 已经登录的cookie:' + req.cookies[cookie_key])
      } else {
        res_raw['code'] = '411'
